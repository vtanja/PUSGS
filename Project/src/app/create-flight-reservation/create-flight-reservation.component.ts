...
export class CreateFlightReservationComponent implements OnInit {
  ...
  constructor(private route:ActivatedRoute, private airlineService:AirlineService, private userService:UserService, private _formBuilder:FormBuilder, private rentCarService:RentCarService) {
     
      this.options = this.userService.getUser().friends.slice();
   ...
   initForm(){
     ...
     console.log(seat);
     let firstName = this.userService.getUser().firstName;
     let lastName = this.userService.getUser().lastName;
    ...
    let passenger1 = new FormGroup({
      ...
      'passporti':new FormControl('', Validators.required)
    });
